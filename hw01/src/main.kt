...
//3 task
fun <B> Tree.fold(fEmpty : B, function : (Int) -> B, fNode : (B, B, B) -> B) : B {
    when (this) {
        is Empty -> return fEmpty
        is Leaf  -> return function(value)
        is Node  -> {
            return fNode(function(value),
                    left.fold(fEmpty, function, fNode),
                    right.fold(fEmpty, function, fNode))
        }
