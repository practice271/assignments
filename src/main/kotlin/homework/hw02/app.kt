...
class computerNet(OSlist : List<String>, val labels : Array<Boolean>, Edges : List<Pair<Int, Int>>) {
    val size = OSlist.size()
    val vertix = Array(size, { i -> Computer(i, OSlist[i]) })
    private  val matrix = Array(size, { i -> Array(size, { i -> false }) })
...
class Net(private val net : computerNet) {
    private var move = 0
    private val n = net.size - 1
